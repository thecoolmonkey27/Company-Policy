...
@onready var ButtonEmail = $ButtonEmail
...
var tab_mail = preload("res://Scenes/tab_mail.tscn")
var tab_game = preload("res://Scenes/tab_game.tscn")
...
func swtichTabToChat():
	remove_child(tab_instance)
	tab_instance = tab_chat.instantiate()
	add_child(tab_instance)
...
func wheel_pressed(input):
	if Global.left_png == Global.right_png and input == 'yes':
		...
		if Global.progress < Global.progress_max and Global.progress > 0:
			Global.progress -= 1
	if Global.progress == Global.progress_max:
		Global.work_completed.emit()
	load_reports()
